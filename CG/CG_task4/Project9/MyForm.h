				...
				while (in) { // если очередная строка считана успешно
					...
					if ((str.find_first_not_of(" \t\r\n") != string::npos) && (str[0] != '#')) {
						...
						if (cmd == "frame") { // размеры изображения
							s >> UVx >> UVy; // считываем глобальные значение Vx и Vy
							UaspectFig = UVx / UVy;
							float aspectRect = Wx / Wy;
							mat3 T1 = translate(-UVx / 2, -UVy / 2);
							float US = UaspectFig < aspectRect ? Wy / UVy : Wx / UVx;
							mat3 S1 = scale(US, -US);
							mat3 T2 = translate(Wx / 2 + Wcx, Wcy - Wy / 2);
							//float UTx = left;
							//float UTy = US * UVy + top; // смещение в положительную сторону по оси Oy после смены знака
							//initT = translate(UTx, UTy) * scale(US, -US); // преобразования применяются справа налево
