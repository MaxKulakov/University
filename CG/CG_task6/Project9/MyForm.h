...
namespace Project9 {
	...
	vec3 S, P, u; // координаты точки наблюдения точки, в которую направлен вектор наблюдения вектора направления вверх
	...
	float l, r, t, b; // рабочие вспомогательные переменные для значений координат левой, правой, нижней и верхней координаты в СКН
...
private: System::Void MyForm_Paint(System::Object^  sender, System::Windows::Forms::PaintEventArgs^  e) {
	...
	mat3 cdr = cadrRL(vec2(-1.f, -1.f), vec2(2.f, 2.f), vec2(Wcx, Wcy), vec2(Wx, Wy));	// матрица кадрирования
	...
	for (int k = 0; k < models.size(); k++) // цикл по моделям
	{
		...
		for (int i = 0; i < figure.size(); i++)
		{
			...
			vec3 start_3D = normalize(TM * vec4(lines.vertices[0], 1.0));	// начальная точка первого отрезка в трехмерных евклидовых координатах
			vec2 start = normalize(cdr * vec3(vec2(start_3D), 1.f));	// начальная точка первого отрезка в координатах экрана
			for (int j = 1; j < lines.vertices.size(); j++)	// цикл по конечным точкам (от единицы)
			{
				vec3 end_3D = normalize(TM * vec4(lines.vertices[j], 1.0));	// конечная точка отрезка в трехмерных евклидовых координатах
				vec2 end = normalize(cdr * vec3(vec2(end_3D), 1.f));	// конечная точка отрезка в координатах экрана
	...
	private: System::Void MyForm_KeyDown(System::Object^ sender, System::Windows::Forms::KeyEventArgs^ e) {
		...
		case Keys::F: //вращение против часовой стрелки
		{
			if (Control::ModifierKeys == Keys::Shift) {
				...
				vec3 u_new = mat3(M) * vec3(0, 1, 0); // вращение направления вверх
				vec3 P_new = normalize(M * vec4(0, 0, -1, 11)); // вращение точки, в которую смотрит наблюдатель
				T = lookAt(vec3(0, 0, 0), P_new, u_new) * T;
			}
			break;
		}
		case Keys::H: //вращение по часовой стрелке
		{
			if (Control::ModifierKeys == Keys::Shift) {
				mat4 M = rotateP(0.1, vec3(0, 1, 0), vec3(0, 0, -dist)); // матрица вращения относительно точки P
				vec3 u_new = mat3(M) * vec3(0, 1, 0); // вращение направления вверх
		...
		case Keys::X:
		{
			if (Control::ModifierKeys == Keys::Shift)	//уменьшение fovy_work на 0.05
			{
				if (fovy_work >= 0.01)
				{
					fovy_work -= 0.05;
				...
				while (in) // если очередная строка считана успешно
				{
					...
					if ((str.find_first_not_of(" \t\r\n") != string::npos) && (str[0] != '#')) {
						...
						if (cmd == "camera") {	// положение камеры
							...
							figure.push_back(path(vertices, vec3(r, g, b), thickness));	// все точки считаны, генерируем ломаную (path) и кладем ее в список figure
						}
							...
							initM = scale(S, S, S) * translate(-mVcx, -mVcy, -mVcz);	// сдвиг точки привязки из начала координат в нужную позицию после которого проводим масштабирование
							figure.clear();
						}
							...
							M = rotate(theta / 180.f * Math::PI, vec3(nx, ny, nz)) * M;	// добавляем вращение к общему преобразованию
						}
					...
					getline(in, str);	// считываем очередную строку
				}
				initWorkPars();
				Refresh();
			}
