...
void Set::sort()
{
	...
	if (temp->next == NULL && temp->prev == NULL)
		cout << "Множество одноэлементное. Сортировка не требуется\n";
	else
	{
		while (temp->next)
		{
...
int main()
{
	...
	case 1:
	{
		...
		try
		{
				...
				for (int i = 0; i < cnt; i++)
				{
					if (isalpha(k[i])) throw Exception("Ошибка\n");
				}
				//a = k.find_first_of(test);
				//if (a != b) throw Exception("Ошибка111\n");

		/*		for (int i = 0; i < cnt; i++)
				{
					if ((k[i] >= 'a') || (k[i] <= 'z') || (k[i] >= 'A') || (k[i] <= 'Z')) throw Exception("Ошибка ввода\n");
				}*/
				for (int i = 0; i < cnt; i++)
				{
					if (k[i] == ' ')
				...
				if (TmpCnt > n) throw MemoryException("Ошибка1\n");
			/*	string razdel = " ";
				int n = 0;
				int pos = 0;
				n = k.find_first_of(razdel);
				while (n != string::npos)
				{
					string num = k.substr(pos, n - pos);
	...
	case 2:
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			try
			{
				if ((!in) || (!out)) throw InputException("Ошибка ввода\n");
			}
	...
	//	try
	//	{
	//		if (!(cin >> k)) throw Exception("Ошибка");

	//		else if ((!in) || (!out)) throw InputException("Ошибка открытия файла");
	//	
	//	}
	//	catch (Exception)
	//	{
